...
Program SISKO telah menyediakan fasilitas untuk penanganan penilaian siswa yang secara langsung memasukkan nilai ke dalam raport dan siap dicetak. Untuk sistem penilaian siswa, yang dapat melakukan pengisian hanya Guru yang mengajar mata pelajaran. Sistem penilaian telah disesuaikan dengan KBK sehingga masingmasing guru dapat memasukkan deskripsi narasi dari mata pelajaran. Untuk menampilkan data penilaian dapat disesuaikan kembali dengan kebijaksanaan dari masing-masing lembaga pendidikan apakah ingin menampilkan data nilai akhir siswa maupun menampilkan data nilai siswa setiap kali mengadakan test ataupun tugas tertentu
...
function listWords() {
  const paragraphs = p.innerHTML.split('\n')
  const words = paragraphs.map(paragraph => paragraph.split('.').join('').split(',').join('').split(' '))
  let wordList = []
  words.forEach(word => {
    word.forEach(w => wordList.push(w))
  })
  wordList = [...new Set(wordList.map(word => word.toLowerCase()))]
  wordList = wordList.sort((a, b) => a.localeCompare(b))
  wordList = wordList.filter(word => isNaN(word))
  
  const ul = document.querySelector('#ListWords ul')
  const liS = wordList.map(word => `<li>${word}</li>`)
  ul.innerHTML = liS.join('')
}

document.querySelector('input[name="search"').addEventListener('input', (e) => {
  const paragraphs = p.innerHTML.split('\n')
  const searchString = paragraphs.map(paragraph => {
    const texts = paragraph.split('.').join('').split(',').join('').split(' ')
    return texts.filter(text => text.toLowerCase().includes(e.target.value.toLowerCase())).length
  });

  const lotsOfText = searchString.reduce((previousValue, currentValue) => previousValue + currentValue, 0)
  document.querySelector('#Searching span').innerHTML = lotsOfText
})

document.querySelector('#ChangeWord button').addEventListener('click', (e) => {
  const currentWord = document.querySelector('input[name="currentWord"').value
  const newWord = document.querySelector('input[name="newWord"').value

  const currentText = p.innerHTML.split(new RegExp(currentWord, 'gi')).join(newWord)
